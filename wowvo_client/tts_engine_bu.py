    ...
    def synthesize(
        ...
        protect: float = 0.25
            ...
            if os.path.isfile(rvc_model_path):
                ...
                return wavfile.write(output_path, sr, audio), output_path
            else:
                print(f"⚠️ No RVC model found for {voice_name} at {rvc_model_path}")
                #make sure nothing weird happens here with the sampling rate and re-exporting, etc.
                return sf.write(output_path, final_audio, 24000), output_path  # XTTS uses 24kHz
